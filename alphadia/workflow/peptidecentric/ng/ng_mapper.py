...
def set_ng_thread_count(thread_count: int) -> None:
    """Set the number of threads for NG computations if not done yet."""
    if get_num_threads() != thread_count:
        # set_num_threads must be called only once, and must be the first call to alphadia_search_rs
