...
@alphatims.utils.njit
def quadrupole_transfer_function(
    ...
    isotope_mz : np.ndarray
        Array of precursor isotope m/z values, shape (n_precursors, n_isotopes)

    Returns
    -------

    intensity : np.ndarray
        Array of predicted intensity values, shape (n_precursors, n_isotopes, n_observations, n_scans)

    """

    n_precursors = isotope_mz.shape[0]
    n_isotopes = isotope_mz.shape[1]
    ...
    observation_column = utils.tile(np.repeat(observation_indices, n_scans), n_precursors * n_isotopes)
    scan_column = utils.tile(scan_indices, n_precursors * n_isotopes * n_observations)

    intensity = quadrupole_calibration_jit.predict(observation_column, scan_column, mz_column)
    return intensity.reshape(n_precursors, n_isotopes, n_observations, n_scans)
...
def calculate_template(
    ...
    precursor_mz = precursor_mz.reshape(n_precursors, n_isotopes, 1, n_scans, n_frames)

    # expand add frame dimension to qtf
    qtf_exp = np.expand_dims(qtf, axis=-1)

    isotope_exp = isotope_intensity.reshape(1,-1,1,1,1)

    template = precursor_mz * isotope_exp * qtf_exp
    template = template.sum(axis=1) 
