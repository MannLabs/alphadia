...
class TimsTOFTransposeJIT(object):
    ...
    def get_scan_indices(
        ...
        optimal_scan_limits = np.array([scan_index[0], scan_index[0]-optimal_len], dtype=np.int64)
    ...
    def get_tof_indices_tolerance(
            ...
            return utils.make_slice_2d(
                self.get_tof_indices(
                    mz_limits
                )
            )
    ...
    def assemble_push(
        ...
        # cycle values
        precursor_cycle_start = int(frame_limits[0,0]-self.zeroth_frame)//self.cycle.shape[1]
        precursor_cycle_stop = int(frame_limits[0,1]-self.zeroth_frame)//self.cycle.shape[1]
                ...
                while (idx < stop) and (i < len(push_query)):
                        ...
                        if push_query[i] < self.push_indices[idx]:
                            ...
                            if push_query[i] == self.push_indices[idx]:
                                ...
                                precursor_cycle_index = (frame_index-self.zeroth_frame)//self.cycle.shape[1]
                                ...
                                accumulated_dim1 = dense_output[1,j,relative_precursor_index[i],relative_scan,relative_precursor]

                                new_intensity = self.intensity_values[idx]

                                if absolute_masses:
                                    new_dim1 = (accumulated_dim1 * accumulated_intensity + new_intensity * measured_mz_value)/(accumulated_intensity + new_intensity)

                                else:
                                    new_error = (measured_mz_value - library_mz_value) / library_mz_value * 10**6
                                    new_dim1 = (accumulated_dim1 * accumulated_intensity + new_intensity * new_error)/(accumulated_intensity + new_intensity)
                                
                                dense_output[0,j,relative_precursor_index[i],relative_scan,relative_precursor] = accumulated_intensity + new_intensity
                                dense_output[1,j,relative_precursor_index[i],relative_scan,relative_precursor] = new_dim1
    ...
    def get_dense(
        ...
        tof_limits = utils.make_slice_2d( 
            self.get_tof_indices( 
                utils.mass_range(
                    mz_values, mass_tolerance
                )
            )
        )

        mz_mask = self.cycle_mask(
            quadrupole_mz,
            custom_cycle
        )
