...
def perform_protein_fdr(psm_df):
    ...
    protein_features = fdr.get_q_values(protein_features, 
                                        score_column = 'proba',
                                        decoy_column = 'decoy',
                                        qval_column = 'pg_qval')
    ...
    return pd.concat([
        psm_df[psm_df['decoy'] == 0].merge(
            protein_features[protein_features['decoy'] == 0][['proteins','pg_qval']], 
            on='proteins', 
            how='left'
        ),
        psm_df[psm_df['decoy'] == 1].merge(
            protein_features[protein_features['decoy'] == 1][['proteins','pg_qval']], 
            on='proteins', 
            how='left'
        ),
    ])
