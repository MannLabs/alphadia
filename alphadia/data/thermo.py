...
class ThermoJIT(object):
    ...
    def get_dense(
        ...
        mz_query_slices = utils.mass_range(
            mz_query_list, mass_tolerance
        )
        ...
        precursor_idx_list = calculate_valid_scans(
            quadrupole_mz,
            self.cycle
        )
            ...
            for j, precursor_idx in enumerate(precursor_idx_list):
                ...
                for k, (mz_query_start, mz_query_stop) in enumerate(mz_query_slices):
                    ...
                    rel_idx = np.searchsorted(
                        self.mz_values[idx:peak_stop_idx],
                        mz_query_start,
                        'left'
                    )
                    ...
                    while idx < peak_stop_idx and self.mz_values[idx] <= mz_query_stop:
                        ...
                        if absolute_masses:
                            new_dim1 = (accumulated_dim1 * accumulated_intensity + new_intensity * new_mz_value+ 1e-6)/(accumulated_intensity + new_intensity + 1e-6)
                        else:
                            new_error = (new_mz_value - mz_query_list[k]) / mz_query_list[k] * 10**6
                            new_dim1 = (accumulated_dim1 * accumulated_intensity + new_intensity * new_error+ 1e-6)/(accumulated_intensity + new_intensity+ 1e-6)
