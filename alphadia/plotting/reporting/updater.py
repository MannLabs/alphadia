...
def print_w_style(string:str, style:str,level:int=0, last_item:bool =False)->None:
    """
    ...
    if style == "update":
        ...
        #red color
        style = "\033[31m " + symbol
    #Print with tree structure using level and color
    
    print(f"{' │   '*level}{style} {string}\033[0m")


def print_recursively(config:Union[Dict[str,Any],List[Any]], level:int=0, style:str="auto", last_item:bool= False)->None:
    """
    ...
    if isinstance(config,tuple):
        print_w_style(f"{config[0]} ({config[1]})", style,level, last_item)
        return
    if not isinstance(config, dict) and not isinstance(config,list):
        print_w_style(f"{config}", style,level, last_item)
        return
    for key, value in config.items():
        is_last_item = key == list(config.keys())[-1]
        if isinstance(value, dict):
            print_w_style(f"{key}", style,level, is_last_item)
            print_recursively(value, level+1,style, is_last_item)
        elif isinstance(value, tuple):
            print_w_style(f"{key} : {value[0]} ({value[1]})", style,level, is_last_item)
        elif isinstance(value,list):
            print_w_style(f"{key}", style,level, is_last_item)
            for item in value:
                is_last_item = item == value[-1]
                print_recursively(item, level+1,style, is_last_item)
        else:
            print_w_style(f"{key} : {value}", style,level, is_last_item)


def translate_config(default_config:Union[Dict[str,Any],List[Any]], name:str)->Union[Dict[str,Any],List[Any]]:
    """
...
def translate_config_back(config:Union[Dict[str,Any],List[Any]]):
    ...
    """
    if not isinstance(config, dict) and not isinstance(config, list) and isinstance(config, tuple):
        return config
...
def update_recursive(config:Dict[str,Any], experiment_configs:List[Union[Dict[str,Any],List[Any]]], level:int=0,print_output:bool=True, is_leaf_node:bool = False)->Union[Dict[str,Any],List[Any]]:
    """
    ...
    if isinstance(default_config, tuple):
        if print_output:
            if parent_key is None:
                print_w_style(f"{default_config[0]} ({default_config[1]})", "auto",level, is_leaf_node)
            else:
                print_w_style(f"{parent_key} : {default_config[0]} ({default_config[1]})", "auto",level-1, is_leaf_node)
        ...
        for experiment_config in experiment_configs:
            if default_config[0] != experiment_config[0]:
                    ...
                    new_value = experiment_config if experiment_config[0] != new_value[0] else new_value #
        # If we have a new value, print it
        if new_value != default_config and print_output:
            if parent_key is None:
                print_w_style(f"{new_value[0]} ({new_value[1]})", "update",level, is_leaf_node)
            else:
                print_w_style(f"{parent_key} : {new_value[0]} ({new_value[1]})", "update",level-1, is_leaf_node)
        return new_value
    ...
    if isinstance(default_config, list):
        for i, default_value in enumerate(default_config):
            ...
            default_config[i] = update_recursive({'key': None, 'value':default_value}, potential_config_updates, level,print_output, is_last_item)
        return default_config
    
    for default_key, default_value in default_config.items():       
        if not isinstance(default_value,tuple): #If the default value is not a leaf node, print it's key on separate line
            print_w_style(f"{default_key}", "auto",level, is_leaf_node)
        ...
        default_config[default_key] = update_recursive({'key':default_key, 'value':default_value}, potential_config_updates, level+1,print_output, is_last_item)
    return default_config



def recursive_fill_table(df:'pandas.DataFrame', experiment_name:str, parent_key:str, value:Any)->None:
    """
    ...
    if isinstance(value, dict):
        ...
        for i, value in enumerate(value):
            recursive_fill_table(df, experiment_name, parent_key+"["+str(i)+']', value)
    else:
...
def get_update_table(default_config:'Config',configs:List['Config']) -> 'pandas.DataFrame': 
    """
...
class Config:
    ...
    def update(self, experiments:List['Config'], print_modifications:bool=True):
        ...
        if len(experiments) > 0:
            ...
            self.translated_config = update_recursive({"key":"", "value": self.translated_config}, translated_experiments , print_output=print_modifications)
