...
def test_fasta_digest():
    ...
    plan = planning.Plan(tempdir, fasta_path_list=[common_contaminants], config={"library_prediction":{"predict":True}})
    ...
    plan = planning.Plan(tempdir, library_path=speclib_path, config={"library_prediction":{"predict":True}})
    assert len(plan.spectral_library.precursor_df) > 0
    assert len(plan.spectral_library.fragment_df) > 0

    # load existing library without predict
    plan = planning.Plan(tempdir, library_path=speclib_path, config={"library_prediction":{"predict":False}})
    assert len(plan.spectral_library.precursor_df) > 0
    assert len(plan.spectral_library.fragment_df) > 0