...
class TestIntensityDriftCorrector:
    ...
    def test_merges_sparse_bins_and_appends_trailing_ions(self):
        """Given ions spread thinly along RT, when binned, then consecutive bins are merged until they hold enough ions and the trailing ions join the last bin."""
        # Given: 250 ions one second apart, 10 s bins hold 10 ions each
        rt = np.arange(250, dtype=np.float64)
        deviation = np.where(rt < 100, 0.1, 0.5)
        corrector = IntensityDriftCorrector(
            pd.DataFrame(),
            bin_seconds=10,
            statistic=IntensityDriftStatistic.MEDIAN,
            min_ions_per_bin=100,
        )

        # When
        bin_rt, bin_location = corrector._binned_location(rt, deviation)

        # Then: [0, 100) and [100, 250) with the trailing 50 ions merged into the second bin
        np.testing.assert_allclose(bin_rt, [49.5, 174.5])
        np.testing.assert_allclose(bin_location, [0.1, 0.5])
