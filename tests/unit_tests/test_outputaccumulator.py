...
from alphadia.outputaccumulator import SpecLibFlatFromOutput
...
def prepare_input_data():
    ...
    for i,raw_folder in enumerate(raw_folders):
        ...
        fragment_dfs[i].to_csv(os.path.join(raw_folder, "frag.tsv"), sep="\t", index=False)
...
def test_complete_output_accumulation():
    ...
    built_lib.load_hdf(os.path.join(temp_folder, f"{output.TRANSFER_OUTPUT}.hdf"), load_mod_seq=True)
    
    # Then: all unique precursors should be in the built library
    number_of_unique_precursors = len(np.unique(np.concatenate([psm_df["precursor_idx"].values for psm_df in psm_dfs])))
    assert len(np.unique(built_lib.precursor_df["precursor_idx"])) == number_of_unique_precursors, f"{len(np.unique(built_lib.precursor_df['precursor_idx']))} != {number_of_unique_precursors}"
...
def test_selection_of_precursors():
    ...
    for precursor_idx in np.unique(built_lib.precursor_df["precursor_idx"]):
        selected_probas = built_lib.precursor_df[built_lib.precursor_df["precursor_idx"] == precursor_idx]["proba"].values
        selected_probas = np.sort(selected_probas)
        all_probas = np.concatenate([psm_df[psm_df["precursor_idx"] == precursor_idx]["proba"].values for psm_df in psm_dfs if precursor_idx in psm_df["precursor_idx"].values])
        target_kept_probas = np.sort(all_probas)[:keep_top]
        np.testing.assert_almost_equal (selected_probas, target_kept_probas, decimal=5), f"{selected_probas} != {target_kept_probas}"
...
def test_keep_top_constraint():
    ...
    for precursor_idx in np.unique(built_lib.precursor_df["precursor_idx"]):
        assert len(built_lib.precursor_df[built_lib.precursor_df["precursor_idx"] == precursor_idx]) <= keep_top, f"{len(built_lib.precursor_df[built_lib.precursor_df['precursor_idx'] == precursor_idx])} != {keep_top}"
