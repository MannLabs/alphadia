...
if __name__ == "__main__":
    ...
    raw_location = os.path.join(
        output_dir, "20200827_TIMS04_EVO07_AnBr_1ng_dia_rep01_400s_30min_S1-D1_1_2944.d"
    )
    ...
    precursors_flat, fragments_flat = (
        plan.speclib.precursor_df,
        plan.speclib.fragment_df,
    )

    while initial_ms1_error >= target_ms1_error or iteration < 1:
        logging.info(
            f"Starting Iteration {iteration}, RT error {initial_rt_error:.2f}s,MS1 error {initial_ms1_error:.2f} ppm, MS2 error {initial_ms2_error:.2f} ppm"
        )
        ...
        features_df["decoy"] = precursors_flat["decoy"].values[
            features_df["index"].values
        ]
        features_df["charge"] = precursors_flat["charge"].values[
            features_df["index"].values
        ]
        ...
        if (
            ...
            and initial_ms2_error == target_ms2_error
        ):
