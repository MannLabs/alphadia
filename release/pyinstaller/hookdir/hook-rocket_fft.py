...
if dist_files is not None:
    ...
    runtime_dll_files = [f for f in dist_files if f.as_posix().endswith('.so') or f.as_posix().endswith('.dll')]
    print('runtime_dll_files:', runtime_dll_files)
    collected_runtime_files = [(runtime_dll_file.locate(), runtime_dll_file.parent.as_posix()) for runtime_dll_file in runtime_dll_files]
