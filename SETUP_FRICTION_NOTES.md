...
|------|--------|
| Clone repo | ✅ OK |
| Install conda (Miniforge) | ✅ OK (had to install manually — not present) |
| Create env `alphadia` (py3.11) | ✅ OK |
| `pip install -e ".[stable,development]"` | ✅ OK |
| `alphadia --check` | ✅ OK (but Thermo/Sciex warnings buried — see #4) |
| Install Mono | ✅ OK (required for `.raw`, see #5) |
| Download test data (~3 GB) | ✅ OK |
| **AlphaDIA search (`basic`)** | ✅ **OK** — 101,529 precursors, 8,368 proteins, ~3.2 min, exit 0 |
| **e2e metrics step (`calc_metrics.py`)** | ❌ **FAILED** — `stat.parquet` not found + column schema drift (see #11), but masked by exit 0 |

**Net:** The core software installs and runs a real DIA search correctly. The friction is almost entirely in (a) prerequisites not being spelled out for a clean machine (conda, Mono) and (b) the e2e *metrics/validation* harness having drifted from the current output schema — which undermines its value as the "did my setup work?" check.

### Top 3 fixes to reduce setup friction
1. **Fix the e2e metrics harness (#11)** — it's the prescribed "does it work?" test and it currently fails silently. Highest impact.
2. **Add an explicit prerequisites block** to the install/quickstart docs: install Miniforge/Miniconda (link), and install Mono for `.raw` files — before the `pip install` step (#1, #5).
3. **Fix `cd test` → `cd tests` typo in the CLI quickstart (#6)** — a one-character bug that breaks the very first documented command.
